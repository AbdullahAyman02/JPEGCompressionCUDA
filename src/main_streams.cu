...
int main(int argc, char **argv)
{
    ...
    const int NUM_STREAMS = 8; // Tune as needed
    int blocks_per_stream = (num_blocks + NUM_STREAMS - 1) / NUM_STREAMS;
    ...
    // Per-stream device pointers and compressed data
    ...
    for (int s = 0; s < NUM_STREAMS; ++s) {
        ...
        cudaMalloc(&d_input[s], nblocks * 192 * sizeof(float));
